...
from allennlp.data.tokenizers import (
    Token,
    PretrainedTransformerTokenizer, SpacyTokenizer, WhitespaceTokenizer,
)
...
@DatasetReader.register("copynet_ccks2021")
class CCKS2021DatasetReader(DatasetReader):
    """
    ...
    def _read_ccks2021(self, file_path):

        df = pd.read_excel(file_path, dtype=str).fillna('')
        if '用户问题' in df.columns.tolist():
            for item in self._read_ccks2021_train(df):
                yield item
        else:
    ...
    def _read_ccks2021_train(self, df):
        df['用户问题'] = df['用户问题'].str.lower()
        df['属性名'] = df['属性名'].str.lower()
        df['实体'] = df['实体'].str.lower()
        df['约束属性名'] = df['约束属性名'].str.replace('｜', '|').str.lower()
        df['约束属性值'] = df['约束属性值'].str.replace('｜', '|').str.lower()
        df['约束算子'] = df['约束算子'].str.replace('｜', '|').str.lower()
        df['答案'] = df['答案'].str.replace('｜', '|').str.lower()
        ...
        for item in df.to_dict(orient='records'):
            ...
            if item['答案类型'] == '比较句':
                try:
                    a, b = item['属性名'].strip().split('-')
                    script = "SELECT #### WHERE {{ <{entity}> <{relation}> ?name . ?name <{subrelation}> ?value ".format(
                        entity=item['实体'].strip(),
                        relation=mapping_relation(a.strip()),
                        subrelation=b.strip())
                except:
                    script = "SELECT #### WHERE {{ <{entity}> <{relation}> ?value ".format(
                        entity=item['实体'].strip(),
                        relation=item['属性名'].strip())

                if item['约束算子']:
                    if item['约束算子'] in ['min', 'max']:
                        script += f" . ?name <{item['约束属性名']}> ?a"
                        script += f" . FILTER ( {item['约束算子']}(?a) )"
                    else:
                        queries = []
                        filters = []
                        ops = {}
                        for i, (o, v, k) in enumerate(zip(item['约束算子'].strip().split('|'),
                                                          str(item['约束属性值']).strip().split('|'),
                                                          item['约束属性名'].strip().split('|'))):
                            if k == '流量':
                                if v == '20':
                                    v = '20480'
                                elif v == '30':
                            ...
                            if k not in ops:
                                ...
                                ops[k] = p
                                columns.append(p)
                                queries.append(f'?name <{k.strip()}> ?{p.strip()}')
                            else:
                                p = ops[k]
                            filters.append(f"?{p.strip()} {o.strip()} '{v.strip()}'")
                        script += f" . {' . '.join(queries)}"
                        script += f" . FILTER ( {' || '.join(filters)} )"
                script += ' }'
                script = script.replace('####', '?' + ' ?'.join(columns))
                try:
                    q = store.query(script)
                    query_result = parse_data(q, columns)
                    ground_truth = set(item['答案'].strip('|').split('|'))
                ...
                if item['答案类型'] == '并列句':
                    p, q = item['属性名'].strip().split('|')

                    a, b = p.split('-')
                    c, d = q.split('-')

                    old_scripts = [
                        "SELECT #### WHERE {{ <{entity}> <{relation}> ?name . ?name <{subrelation}> ?value ".format(
                            entity=item['实体'].strip(),
                            relation=mapping_relation(a.strip()), subrelation=b.strip()),
                        "SELECT #### WHERE {{ <{entity}> <{relation}> ?name . ?name <{subrelation}> ?value ".format(
                            entity=item['实体'].strip(),
                            relation=mapping_relation(c.strip()), subrelation=d.strip())]
                    ...
                    try:
                        q = store.query(script)
                        query_result = parse_data(q, columns).value.str.strip("'").tolist()
                    except Exception as e:
                        print(e, item, script)
    ...
    def text_to_instance(self, record: Dict) -> Instance:  # type: ignore
        ...
        if target_string is not None:
            ...
            meta_fields['answer'] = record['answer']
        else:
